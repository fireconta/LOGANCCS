...
app.post('/api/login', [
    ...
    try {
        ...
        const isAdmin = username === 'LVz' && await bcrypt.compare(password, await bcrypt.hash(process.env.ADMIN_PASSWORD, 10));
        res.json({ userId: user._id, username: user.username, is_admin: isAdmin });
    } catch (err) {
