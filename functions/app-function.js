const express = require('express');
     ...
     debug('[INFO] Inicializando função serverless'); // Novo log
     ...
     const connectMongoDB = async () => {
       ...
       try {
         ...
         if (!uri) {
           debug('[ERRO] MONGODB_URI não está definido: %s', uri);
           throw new Error('MONGODB_URI não configurado');
         }
         debug('[INFO] Tentando conectar ao MongoDB com URI: %s', uri.replace(/:([^@]+)@/, ':****@'));
